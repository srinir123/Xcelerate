...
public class PaymentImport_Ext_SUMD extends GenericMethods{
	...
	public static void PaymentImport_Ext_SUMD(int i, int j) throws Exception 
	{  	waitForElement(Click_CollPeriodBalances);
	    Click_CollPeriodBalances.click();
	
		
		waitForElement(Dropd_Agreement);
	String Agreement_Name=getData("Agreement_Name","PaymentImport_Ext_SUMD",i,j);

    Dropd_Agreement.sendKeys(Agreement_Name);

	waitForElement(Dropd_BucketID);
	
	String BucketID=getData("BucketID","PaymentImport_Ext_SUMD",i,j);
